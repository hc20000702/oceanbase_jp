...
RC Table::drop()
{
  ...
  fd = ::remove(data_file.c_str());
  if (fd) {
    return RC::IOERR;
  }
  /* delete meta */
  std::string meta_file = table_meta_file(base_dir_.c_str(), name());
  fd = ::remove(meta_file.c_str());
  if (fd) {
    return RC::IOERR;
  }
...
bool Table::is_unique_key(Record *record)
{
  for (Index *index1: indexes_) {
    const IndexMeta &indexMeta = index1->index_meta();
    if (indexMeta.is_unique()) {
      int size = (*(indexMeta.fields())).size();
      ConditionFilter *filters[size];
      for (int i = 0; i < size; i++) {
        //        // 创建record scanner
        ConDesc left, right;
        const FieldMeta *fieldMeta = table_meta_.field_and_index((*(indexMeta.fields()))[i].c_str(), left.attr_i);
        left.is_attr = true;
        left.attr_length = fieldMeta->len();
        left.attr_offset = fieldMeta->offset();
        left.attr_sysnull = table_meta_.null_field()->offset();
        right.is_attr = false;
        right.value = record->data() + fieldMeta->offset();
        int is_null = *(int32_t *)(record->data() + left.attr_sysnull) & (1 << (31 - left.attr_i));
        DefaultConditionFilter *filter = new DefaultConditionFilter;
        filter->init(left, right, fieldMeta->type(), is_null ? NULLS : fieldMeta->type(), EQUAL_TO);
        filters[i] = filter;
      }
      CompositeConditionFilter * compositeConditionFilter = new CompositeConditionFilter;
      compositeConditionFilter->init((const ConditionFilter **)filters, size);
      RecordFileScanner scanner;
      RC rc1 = scanner.open_scan(*data_buffer_pool_, compositeConditionFilter);
      Record record1;
      if (scanner.has_next()) {
        rc1 = scanner.next(record1);
        if (rc1 == RC::SUCCESS) {
          return false;
        }
      }
...
const FieldMeta* Table::findField(const char* fieldName) const{
  return table_meta_.field(fieldName);
}
...
RC Table::insert_record(Trx *trx, const size_t inserts_num, const size_t *offset, const Value *values)
{
  RC rc = RC::SUCCESS;
  for (size_t k = 0; k < inserts_num; k++) {
    ...
    rc = insert_record(trx, offset[k] - left, values + left);
    if (rc != RC::SUCCESS) {
      return rc;
    }
  }
...
RC Table::make_record(int value_num, const Value *values, char *&record_out)
{
  ...
  int32_t null_map = 0b0;

  const int normal_field_start_index = table_meta_.sys_field_num();
  for (int i = 0; i < value_num; i++) {
    null_map = (null_map << 1) + 0;
  ...
  null_map = null_map << (32 - value_num); // 11100...00
  const FieldMeta *field_meta_null = table_meta_.null_field();
  memcpy(record + field_meta_null->offset(), &null_map, field_meta_null->len());
  for (int i = 0; i < value_num; i++) {
    const FieldMeta *field = table_meta_.field(i + normal_field_start_index);
    const Value &value = values[i];
    ...
    if (field->type() == CHARS) { // TEXT的截断在语法分析里
...
RC Table::create_index(Trx *trx, const char *index_name, const char ** attributes, const size_t attribute_num, const size_t is_unique)

{
  ...
  if (table_meta_.index(index_name) != nullptr || table_meta_.find_index_by_fields(attributes, attribute_num)) {
    LOG_INFO("Invalid input arguments, table name is %s, index %s exist or attribute %s... exist index",
             name(), index_name, attributes[0]);
  ...
  // Record record;
  // scanner.get_first_record(&record);
  //
  // insert_index_record_reader_adapter(&record, index_inserter)
...
{
  int n = table_meta_.index_num();
  for (int i = 0; i < n; i++) {
    const IndexMeta *indexMeta = table_meta_.index(i);
    int j = 0;
    for (const std::string &field: *(indexMeta->fields())) {
      ss << table_meta_.name()
         << " | " << std::to_string(!(indexMeta->is_unique()))
         << " | " << indexMeta->name()
         << " | " << std::to_string(++j)
         << " | " << field << std::endl;
    }
...
typedef struct update_context_{
  //含冗余信息，field是多余的
  int i;
  Value value;
...
//
  ...
  //   RC rc2 = delete_entry_of_indexes(record->data(), record->rid(), true);
  //   if (rc2 != RC::SUCCESS) {
  //     LOG_ERROR("Failed to rollback index data when update index entries failed. table name=%s, rc=%d:%s", name(), rc2, strrc(rc2));
  //   }
  //   return rc;
  // }
...
RC Table::rollback_update(Trx *trx, const RID &rid)
{
  ...
  // TODO: delete index, re-update old record
...
Index *Table::find_index_by_fields(const char **fields, size_t length) const
{
  const TableMeta &table_meta = this->table_meta();
  const IndexMeta *index_meta = table_meta.find_index_by_fields(fields, length);
  if (index_meta != nullptr) {
    return this->find_index(index_meta->name());
  }
  return nullptr;
...
IndexScanner *Table::find_index_for_scan(const DefaultConditionFilter &filter)
{
  ...
  if (filter.left_attr() == CHARS && filter.right_attr() == CHARS) {
