...
{
  RC rc = RC::SUCCESS;
  /* Check table_name & find table */
  Table *table = find_table(table_name);
  if (table == nullptr) {
    LOG_WARN("No table named %s", table_name);
  ...
  delete table;
  table = nullptr;
  /* Remove table from DB in memory */
...
RC Db::recover()
{
  ...
  if ((rc = clog_manager_->recover()) == RC::SUCCESS) {
    ...
    for (auto it = mtr_manager->log_redo_list.begin(); it != mtr_manager->log_redo_list.end(); it++) {
      ...
      if (clog_record->get_log_type() != CLogType::REDO_INSERT && clog_record->get_log_type() != CLogType::REDO_UPDATE && clog_record->get_log_type() != CLogType::REDO_DELETE) {
        delete clog_record;
        continue;
      }
