...
public:
  ...
  RC insert_record(Trx *trx, const size_t inserts_num, const size_t *offset, const Value *values);
  // return false if is unique key
  bool is_unique_key(Record *record);
  ...
  RC create_index(Trx *trx, const char *index_name, const char ** attributes, const size_t attribute_num, const size_t is_unique);

  RC create_index_multi(Trx *trx, const char *index_name, const char *attribute_name_group[], int length);

  RC show_index(std::stringstream &ss);
...
public:
  ...
  const FieldMeta* findField(const char*) const;
...
private:
  ...
  RC delete_record_handler(const char *base_dir);
  RC make_record(int value_num, const Value *values, char *&record_out);

public:
  Index *find_index(const char *index_name) const;
  Index *find_index_by_field(const char *field_name) const;
  Index *find_index_by_fields(const char **fields, size_t length) const;
