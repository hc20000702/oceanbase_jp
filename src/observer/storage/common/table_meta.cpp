...
RC TableMeta::init_sys_fields()
{
  ...
  FieldMeta field_meta_trx;
  RC rc = field_meta_trx.init(Trx::trx_field_name(), Trx::trx_field_type(), 0, Trx::trx_field_len(), false, false);
  if (rc != RC::SUCCESS) {
    LOG_ERROR("Failed to init trx field. rc = %d:%s", rc, strrc(rc));
    return rc;
  }
  FieldMeta field_meta_null;
  rc = field_meta_null.init("null_bitmap", INTS, field_meta_trx.len(), sizeof(int32_t), false, false);
  if (rc != RC::SUCCESS) {
    LOG_ERROR("Failed to init nullable field. rc = %d:%s", rc, strrc(rc));
    return rc;
  }
  sys_fields_.push_back(field_meta_trx);
  sys_fields_.push_back(field_meta_null);
  return rc;
}
RC TableMeta::init(const char *name, int field_num, const AttrInfo attributes[])
{
  ...
  for (int i = 0; i < field_num; i++) {
    ...
    rc = fields_[i + sys_fields_.size()].init(attr_info.name, attr_info.type, field_offset, attr_info.length, attr_info.nullable, true);
    if (rc != RC::SUCCESS) {
...
const FieldMeta *TableMeta::field_and_index(const char *name, int &idx) const
{
  int i = -sys_field_num();
  if (nullptr == name) {
    idx = -1;
...
const IndexMeta *TableMeta::find_index_by_field(const char *fields) const
{
  for (const IndexMeta &index : indexes_) {
    ...
    if (std::find(index.fields()->begin(), index.fields()->end(), fields) != index.fields()->end()) {
      return &index;
    }
  }
  return nullptr;
...
const IndexMeta *TableMeta::find_index_by_fields(const char **fields, size_t length) const
{
  for (const IndexMeta &index : indexes_) {
    if (index.fields()->size() != length) {
      continue;
    }
    bool flag = true;  //判断当前index是不是要找的
    for (size_t i = 0; i < length; i++) {
      if (std::find(index.fields()->begin(), index.fields()->end(), (char *)(fields[i])) == index.fields()->end()) {
        flag = false;
        break;
      }
    }
