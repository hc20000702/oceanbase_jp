...
struct ConDesc {
  ...
  int attr_i;       // 如果是属性，表示是第i个属性（从0开始）
  int attr_sysnull; // 如果是属性，表示null_map在记录中的偏移量
...
public:
  ...
  AttrType left_attr() const { return type_left_; }
  AttrType right_attr() const { return type_right_; }

private:
  ConDesc left_;
  ConDesc right_;
  AttrType type_left_ = UNDEFINED;
  AttrType type_right_ = UNDEFINED;
