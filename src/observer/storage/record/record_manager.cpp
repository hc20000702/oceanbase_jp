...
RC RecordPageHandler::recover_update_record(const Record *rec)
{
  // 目前和update_record完全一样
  if (rec->rid().slot_num >= page_header_->record_capacity) {
    LOG_ERROR("Invalid slot_num %d, exceed page's record capacity, page_num %d.",
	      rec->rid().slot_num, frame_->page_num());
    return RC::INVALID_ARGUMENT;
...
RC RecordFileHandler::recover_update_record(const Record *rec)
{
  RC ret = RC::SUCCESS;
  RecordPageHandler record_page_handler;
  ...
  if (ret != RC::SUCCESS) {
    LOG_WARN("failed to init record page handler. page num=%d, rc=%d:%s", rec->rid().page_num, ret, strrc(ret));
    return ret;
  }

  return record_page_handler.recover_update_record(rec);
}
