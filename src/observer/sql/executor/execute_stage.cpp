...
IndexScanOperator *try_to_create_index_scan_operator(Table* table, FilterStmt *filter_stmt,const std::vector<Field>& query_fields)
{
  ...
  for (const FilterUnit * filter_unit : filter_units) {
    if(filter_unit->isScanFilter() == nullptr
        || strcmp(filter_unit->isScanFilter(), table->name()) != 0){
      continue;
    }

    if (filter_unit->comp() == NOT_EQUAL || filter_unit->comp() >= LIKE) {
      continue;
    }

    BaseExpression *left = filter_unit->left();
    BaseExpression *right = filter_unit->right();
    if (left->isId() && right->isConst()) {
    } else if (left->isConst() && right->isId()) {
      std::swap(left, right);
    }else{
      return nullptr;
    }
    ...
    for(auto& it: query_fields){
      if(it.table_name() == ((IdentifierExpression*)left)->getTableName()
          && it.field_name() == ((IdentifierExpression*)left)->getFieldName())
        field = &it;
    }
    if(field == nullptr){
      return nullptr;
  ...
  BaseExpression *left = better_filter->left();
  BaseExpression *right = better_filter->right();
  CompOp comp = better_filter->comp();
  if (left->isConst() && right->isId()) {
    std::swap(left, right);
    switch (comp) {
...
RC ExecuteStage::do_select(SQLStageEvent *sql_event)
{
  ...
  for(Table* t : select_stmt->tables()){
    Operator *scan_oper = try_to_create_index_scan_operator(t, select_stmt->filter_stmt(), select_stmt->query_fields());
    if (nullptr == scan_oper) {
      scan_oper = new TableScanOperator(t);
    }
    select_stmt->addScanOperator(scan_oper);
  }

  QueryOperator* queryOperator;
  rc = select_stmt->generateOperatorFlow(queryOperator);
  ...
  std::stringstream ss;
  relation->printToStream(ss, select_stmt->getTableAliasMap());
...
RC ExecuteStage::do_create_index(SQLStageEvent *sql_event)
{
  ...
  RC rc = table->create_index(nullptr, create_index.index_name, (const char**)create_index.attributes, create_index.attribute_num, create_index.is_unique);
...
RC ExecuteStage::do_insert(SQLStageEvent *sql_event)
{
  ...
  RC rc = table->insert_record(trx, insert_stmt->inserts_num(), insert_stmt->offset(), insert_stmt->values());
  if (rc == RC::SUCCESS) {
    if (!session->is_trx_multi_operation_mode()) {
